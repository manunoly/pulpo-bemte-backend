...
class TareasController extends Controller
{
    public function solicitarTarea(Request $request)
    {
        if ($request) 
        {
            ...
            $limit = date("Y-m-d H:i:s", strtotime($request['fecha_entrega'].' '.$request['hora_fin']. '-2 hours'));
    ...
    public function devuelveTarea()
    {
        ...
        $tarea = Tarea::join('alumnos', 'alumnos.user_id', '=', 'tareas.user_id')
                    ...
                    ->leftJoin('tarea_profesor as tp', 'tp.tarea_id', '=', 'tareas.id')
                    ...
                    'alumnos.apodo AS apodoAlumno', 'profesores.apodo as apodoProfesor', 'tp.inversion as valor_compra')
                    ->first();
        if ($tarea != null)
        {
            $tarea['profAplicados'] = [];
